...
@click.option(
    "--name", help="Custom name for the archive (default: source_name_timestamp)"
)
@click.option("--no-archive", is_flag=True, help="Skip creating the .tar.gz archive")
@click.option(
    "--delete-after-archive",
    is_flag=True,
    help="Delete original directory after archiving",
)
...
@click.option(
    "--storage-provider",
    type=click.Choice(["rclone", "aws", "gcp", "azure"]),
    default="rclone",
    show_default=True,
